...
/**
 * Memoized particle component to prevent unnecessary re-renders when
 * the parent Hero component's visibility state changes.
 */
...
export default function ThreeHero() {
    ...
    // Performance: Only run the animation loop when the component is in the viewport
    // This significantly reduces GPU and CPU usage when the user scrolls past the hero
    useEffect(() => {
        ...
        <div ref={containerRef} className="absolute inset-0 z-0 bg-primary">
            ...
            <div className="absolute inset-0 z-10 w-full h-full opacity-80">
                <Canvas
                    ...
                    dpr={[1, 2]} // Cap DPR to prevent performance drop on high-DPI screens
                    frameloop={isVisible ? 'always' : 'never'} // Idle GPU when off-screen
                >
